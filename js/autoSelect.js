...
function sModTableBrandWrite(res) {
    ...
    document.getElementById('sModHeadBrand').innerHTML = 'Марка автомобиля: ' + abcindex;
...
function sModTableBrandRead() {
    if (!document.getElementById('sModTabBodyBrand')) { //querySelector('#sModTabBodyBrand>tr')) {
...
function sModTableModelRead() {
    if (!document.getElementById('sModTabBodyModel')) { //querySelector('#sModTabBodyModel>tr')) {
...
sModOverCar.onclick = function (e) {
    ...
    if (maindocument) {
        ...
        tr.children[1].innerText+', '+tr.children[2].innerText+'. '+tr.children[3].innerText;
    }
